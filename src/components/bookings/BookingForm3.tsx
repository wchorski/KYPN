...
export function BookingForm3 ({ services, locations, addons, employees, availabilities, gigs, session, prevBooking }:Props) {
  ...
  const reducer = (state:StateRed, action:FormAsideAction):StateRed => {
    ...
    switch (action.type) {
      ...
      case 'SET_SERVICE':
        ...
        const pickedService = services.find(serv => serv.id === pickedServiceId)
        const serviceLocations = locations.filter(loc => loc.services.flatMap(serv => serv.id).includes(pickedServiceId) )
        const locationOpts = serviceLocations.map((obj) => { return {value: obj.id, label: obj.name} } )
  ...
  function findPartialDays(id:string) {
    ...
    if(!selectedEmpl) return dispatchRed({type: 'SET_BLACKOUT_DATES', payload: []})
    ...
    const busyDays = uniqueBusyDays.filter((day) => {
      const openTimes = buisnessTimeStrings.filter((time) => {
        ...
        testEnd.setMinutes(testEnd.getMinutes() + Number(stateRed.service?.durationInHours) * 60)
