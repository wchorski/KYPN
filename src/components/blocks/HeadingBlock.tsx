...
export function HeadingBlock({ id, level, children, textAlign }: Props) {  
  ...
  const dynID = id ? id : children[0] ? children[0].props.node.text : 'no_id'
	...
	return (
		<Element level={level} textAlign={textAlign} id={dynID}>
			{children}
		</Element>
	)
}
...
function Element({ id, level, textAlign, children }: Props) {
	...
	if (level === 4) return <h4 id={id} style={style}>{children}</h4>

	if (level === 5) return <h5 id={id} style={style}>{children}</h5>

	if (level === 6) return <h6 id={id} style={style}>{children}</h6>
